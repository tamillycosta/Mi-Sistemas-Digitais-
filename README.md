...
<div align="justify">
  <p>   </p>
  <p>Para viabilizar a implementação dos filtros utilizando o coprocessador, foi necessário modificá-lo, uma vez que sua versão original não possuía suporte à operação de multiplicação no formato requerido pelos filtros. Especificamente, o coprocessador anterior não implementava a operação de convolução, que é essencial na aplicação de filtros sobre imagens. Assim, a operação de soma existente foi adaptada para executar o produto elemento a elemento entre uma região da imagem e a máscara do filtro, seguida da soma desses produtos. Para isso, o código foi reestruturado de forma que, a cada ciclo de clock, os valores correspondentes da imagem e da máscara fossem extraídos, multiplicados entre si, e os resultados parciais armazenados. Ao final das 25 multiplicações, os valores são somados e o resultado final é limitado a 8 bits (representados com sinal), com verificação de overflow, sendo então atribuído ao barramento de saída.
  </p>
